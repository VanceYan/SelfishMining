...
def getBestUtility(selfishMachineTotal, honestMachineTotal, EBRR, C_op, C_cop):
    # SelfishMining 相关参数
    gamma, steps, limit, rewardExpCount = 0.5, 200, 100000, 10000
    # 开机时间序列
    selfishOntimesList = np.linspace(0, 1, 11)[1:]
    # 自私玩家采用 t = 0 时的自私挖矿对象
    selfishZeroObj = SelfishMining([0], [1], selfishMachineTotal,
                                   [0], [1], honestMachineTotal,
                                   weight=gamma, steps=steps, expCount=limit)
    # 计算转移概率
    selfishZeroObj.getTransferProbability()
    # 进行马尔可夫状态转移
    print("开机时间：0，转移概率：%.4f" % selfishZeroObj.alpha)
    selfishZeroObj.getMarkovState()
    # 设置奖励参数
    selfishZeroObj.rewardParameter(rewardExpCount=rewardExpCount, EBRR=EBRR, op=C_op, cop=C_cop)
    # 自私玩家采用 t = 0 时的效用
    selfishZeroUtility, honestZeroUtility = selfishZeroObj.getUtility()
    # 自私玩家的最优开机时间
    bestOntime = 0
    # 自私玩家的最高效用
    bestUtility = selfishZeroUtility
    # 寻找自私玩家的最优开机时间及对应效用
    for selfishOntime in selfishOntimesList:
        print("\n开机时间: %.1f " % selfishOntime)
        objSelfish = SelfishMining([selfishOntime], [1], selfishMachineTotal,
                                   [0], [1], honestMachineTotal,
                                   weight=gamma, steps=steps, expCount=limit)
        # 计算转移概率
        objSelfish.getTransferProbability()
        print("转移概率: %.4f " % objSelfish.alpha)
        # 进行马尔可夫状态转移
        objSelfish.getMarkovState()
        # 设置奖励参数
        objSelfish.rewardParameter(rewardExpCount=rewardExpCount, EBRR=EBRR, op=C_op, cop=C_cop)
        # 计算绝对效用
        selfishUtility, honestUtility = objSelfish.getUtility()
        print("自私玩家的最终效用：%.6f" % selfishUtility)
        # 记录最优效用及对应的开机时间
        if selfishUtility > bestUtility:
            bestUtility = selfishUtility
            bestOntime = selfishOntime
    print("\n ☞ 最佳效用 %.4f、对应开机时间 %.1f" % (bestUtility, bestOntime))
    return bestOntime, bestUtility, selfishZeroUtility


def getResult(C_op, C_cop, savePath, machine_Total = 256):
    # 自私玩家的矿机比例
    selfish_OnProportions = np.linspace(0.1, 0.5, 41)

    # EBRR 列表
    EBRRList = [0, 0.5, 1, 1.5, 2]

    # 记录自私玩家的相对额外效用
    selfishRelativeUtilitys = []
    for i in range(len(selfish_OnProportions)):
        selfishRelativeUtilitys.append([])
    for portionIndex, selfishPortion in enumerate(selfish_OnProportions):
        print("比例为 %.2f 时："%selfishPortion)
        # 自私玩家的矿机数
        selfish_Total = selfishPortion * machine_Total
        # 诚实玩家的矿机数
        honest_Total = machine_Total - selfish_Total
        # 记录五条曲线的数据
        for ebrr in EBRRList:
            print("EBRR = %.1f  C_op=%.2f  C_cop=%.2f"%(ebrr, C_op, C_cop))
            # 自私玩家采用最优开启时间时的效用
            print("\n寻找最优额外效用：\n")
            selfishBestOntime, selfishBestUtility, selfishZeroUtility = getBestUtility(selfish_Total, honest_Total, ebrr, C_op, C_cop)
            # 记录数据（相对额外效益 in %）
            extraUtility = abs(100*(selfishBestUtility-selfishZeroUtility)/selfishZeroUtility)
            selfishRelativeUtilitys[portionIndex].append((extraUtility))
            print("\nEBRR = %.1f：最佳开机时间 [%.2f]、最佳效用 [%.4f]、0 时效用 [%.4f]、相对效用 [%.4f]\n\n" %
                  (ebrr, selfishBestOntime, selfishBestUtility, selfishZeroUtility, extraUtility))

    # 将结果写入文件
    with open(savePath + "RelativeUtilitys.csv", "w") as f:
        for portionList in selfishRelativeUtilitys:
            utilityNum = len(portionList)
            for index, utility in enumerate(portionList):
                if index != utilityNum - 1:
                    f.write(str(utility) + ",")
                else:
                    f.write(str(utility) + "\n")
    print("相对额外效用写入完毕")
    print(selfishRelativeUtilitys)


if __name__ == '__main__':
    # 实验 6
    getResult(0.02, 0, "../ExperimentData/exp6_1_")
    getResult(0.01, 0.01, "../ExperimentData/exp6_2_")
    getResult(0, 0.02, "../ExperimentData/exp6_3_")
