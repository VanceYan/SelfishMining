...
class SelfishMining:
    ...
    def __getSelfishExpense(self, T):
        ...
        :return:
        '''
        return (self.C_cop + self.C_op) * (self.selfish_Total * T)
...
def getResult(C_op, C_cop, savePath, machineTotal = 256, ConvergenceThreshold = 0.02):
    # SelfishMining 相关参数
    gamma, steps, limit = 0.5, 200, 100000
    # 自私玩家的相对算力
    selfishPortions = np.linspace(0, 0.5, 101)[1:]
    # 设置初值
    EBRRList = [0, 1, 2]
    # 存放结果的数组
    Utilitys = [[], [], []]
    for selfishPortion in selfishPortions:
        ...
        print("\n自私玩家相对算力：%.3f（对应开机数为：%d）" % (selfishPortion, selfishMachine))
        # 构建挖矿对象
        obj = SelfishMining([0], [1], selfishMachine,
                            [0], [1], honestMachine,
                            gamma, steps, limit)
        ...
        obj.getMarkovState()
        # 设置奖励参数
        for index, ebrr in enumerate(EBRRList):
            obj.rewardParameter(rewardExpCount=10000, EBRR=ebrr, op=C_op, cop=C_cop)
            # 计算绝对效用
            selfishUtility, honestUtility = obj.getUtility()
            print("EBRR=%d"%ebrr)
            print("自私玩家效用：%.6f"%selfishUtility)
            print("诚实玩家效用：%.6f"%honestUtility)
            # 保存结果
            Utilitys[index].append([selfishUtility, honestUtility])
...
if __name__ == '__main__':
    # 添加实验
    ## high_op
    # getResult(0.02, 0, "../ExperimentData/exp_extra3_1.csv")
    #
    # # mid_op
    # getResult(0.01, 0.01, "../ExperimentData/exp_extra3_2.csv")
    #
    # # low_op
    # getResult(0, 0.02, "../ExperimentData/exp_extra3_3.csv")
    #
    ## no_op
    # getResult(0, 0.01, "../ExperimentData/exp_extra3_4.csv")

    # no_cop
    getResult(0.01, 0, "../ExperimentData/exp_extra3_5.csv")
