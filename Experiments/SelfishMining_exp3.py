...
def getOntimeAndPortions(playerIndex, PlayerOntimesList, PlayerMachineTotal):
    '''
    根据传递玩家的序号求出由其余玩家的开机时间和数量序列构成的上帝视角序列
    :param playerIndex:
    :param PlayerMachineTotal:
    :param PlayerOntimesList:
    :return:
    '''
    # SelfishMining 相关参数
    gamma, steps, limit= 0.5, 200, 5000
    # 取出其余玩家的开机时间和数量
    otherPlayerOntimes, otherPlayerMachines = [], []
    for index in range(4):
        if index == playerIndex:
            continue
        otherPlayerOntimes.append(PlayerOntimesList[index][-1])
        otherPlayerMachines.append(PlayerMachineTotal[index])
    # 融合 3 个玩家的开机时间和比例
    # 第一次融合
    obj = SelfishMining([otherPlayerOntimes[0]], [1], otherPlayerMachines[0],
                        [otherPlayerOntimes[1]], [1], otherPlayerMachines[1],
                        gamma, steps, limit)
    # 计算转移概率（这一步已经实现了玩家的开机序列融合）
    obj.getTransferProbability()
    # 第二次融合
    obj = SelfishMining(obj.god_OnTimes, obj.god_OnProportions, otherPlayerMachines[0] + otherPlayerMachines[1],
                        [otherPlayerOntimes[2]], [1], otherPlayerMachines[2],
                        gamma, steps, limit)
    # 计算转移概率（这一步已经实现了玩家的开机序列融合）
    obj.getTransferProbability()
    return obj.god_OnTimes, obj.god_OnProportions


def findBestOntimes(playerIndex, selfMachineTotal, objectOntimeList, objectOnProportionList, objectMachineTotal):
    '''
    根据现有开机策略寻找此刻的最佳开机时间
    :param playerIndex: 角色序号
    ...
    '''
    # SelfishMining 相关参数
    gamma, steps, limit, ebrr, op, cop = 0.5, 200, 100000, 2, 0.02, 0
    ...
    if playerIndex == 0:
        for PlayerOntime in PlayerOntimes:
            objSelfish = SelfishMining([PlayerOntime], [1], selfMachineTotal, objectOntimeList,
                                        objectOnProportionList, objectMachineTotal, gamma, steps, limit)
            # 计算转移概率（这一步已经实现了玩家的开机序列融合）
            ...
            print("    Search best on time, schedule：{:.2f}%".format(cnt * 100/len(PlayerOntimes)))
            cnt += 1
    else:
        for PlayerOntime in PlayerOntimes:
            objHonest = SelfishMining(objectOntimeList, objectOnProportionList, objectMachineTotal,
                                       [PlayerOntime], [1], selfMachineTotal,gamma, steps, limit)
            # 计算转移概率（这一步已经实现了玩家的开机序列融合）
...
def getResult(pot1, pot2, pot3, pot4, savePath, machineTotal = 256, ConvergenceThreshold = 0.02):
    '''
    四个玩家的机器数占比
    :param pot1: 玩家 1 （自私玩家）
    :param pot2: 玩家 2 （诚实玩家）
    :param pot3: 玩家 3 （诚实玩家）
    :param pot4: 玩家 4 （诚实玩家）
    :param machineTotal: 机器总数
    :return:
    '''
    # 各玩家的机器总数
    PlayerMachineTotal = [
        int(pot1 * machineTotal),
        int(pot2 * machineTotal),
        int(pot3 * machineTotal),
        0
    ]
    PlayerMachineTotal[3] = machineTotal - sum(PlayerMachineTotal)

    # 设置随机数种子（保证后续实验总是用的同一数据）
    random.seed(6)
    # 随机生成 3 个诚实玩家的开机时间、比例和总数
    honestPlayer1Ontime = random.randint(1, 100) / 100
    honestPlayer2Ontime = random.randint(1, 100) / 100
    honestPlayer3Ontime = random.randint(1, 100) / 100
    # 全部玩家的开机时间
    PlayerOntimesList = [[], [honestPlayer1Ontime], [honestPlayer2Ontime], [honestPlayer3Ontime]]
    ...
    while True:
        print("第 %d 次迭代" % cnt)
        # 计算指定角色的最优开机时间
        for roleIndex in range(4):
            # 构建其他玩家的开机时间和比例序列
            lastOtherOnTimes, lastOtherOnProportions = getOntimeAndPortions(roleIndex, PlayerOntimesList, PlayerMachineTotal)
            # 寻找指定角色的最优开机时间
            bestOntime = findBestOntimes(roleIndex, PlayerMachineTotal[roleIndex], lastOtherOnTimes,
                            lastOtherOnProportions, machineTotal - PlayerMachineTotal[roleIndex])
            # 为当前角色添加最优开机时间
            PlayerOntimesList[roleIndex].append(bestOntime)
            print(" 玩家 %d 的最佳开机时间序列 %s"%(roleIndex+1, PlayerOntimesList[roleIndex]))
        cnt += 1
        # 判断各角色的开机时间是否收敛
        for roleIndex in range(4):
            if len(PlayerOntimesList[roleIndex]) > 1:
                if abs(PlayerOntimesList[roleIndex][-1] - PlayerOntimesList[roleIndex][-2]) <= ConvergenceThreshold:
                    ConvergenceList.append(roleIndex)
        # 全部玩家都收敛时，退出循环
        if len(ConvergenceList) == 4:
            break
        # 否则重置收敛序列
        ConvergenceList = []
    ...
    with open(savePath, "w") as f:
        for playerOntimes in PlayerOntimesList:
            playerOntimesLen = len(playerOntimes)
            for index, onTime in enumerate(playerOntimes):
                if index != playerOntimesLen-1:
                    f.write(str(onTime) + ",")
                else:
                    f.write(str(onTime) + "\n")
...
if __name__ == '__main__':
    ...
    # 并行计算
    print("图 1")
    getResult(0.1, 0.2, 0.2, 0.5, "../ExperimentData/exp3_1.csv")
    # print("图 2")
    # getResult(0.2, 0.2, 0.2, 0.4, "../ExperimentData/exp3_2.csv")
    # print("图 3")
    # getResult(0.3, 0.1, 0.2, 0.4, "../ExperimentData/exp3_3.csv")
