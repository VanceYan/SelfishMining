...
def findBestOntimes(playerClass, machineTotal, objectOntime, objectMachineTotal, EBRR, C_op, C_cop):
    '''
    根据现有开机策略寻找此刻的最佳开机时间
    :param playerClass: 角色类型
    :param achineTotal: 角色总机器数
    ...
    if playerClass == 0:
        ...
        for PlayerOntime in PlayerOntimes:
            objSelfish = SelfishMining([PlayerOntime], [1], machineTotal,
...
def getResult(C_op, C_cop, machineTotal = 256, ConvergenceThreshold = 0.05):
    '''
    自私玩家以及诚实玩家的最优开启时间
    :param C_op:
    :param C_cop:
    ...
    ebrr = 0.5
    while True:
        print("当前 EBRR=%.1f"%ebrr)
        FLAG = False
        for selfishPortion in selfishPortions:
            print("比例为 %.2f " % selfishPortion)
            # 全部玩家的机器总数序列
            ...
            while True:
                ...
                if cnt == 20:
                    break
            # 如果自私玩家的开机时间不为 0，则算力资源利用率不为 1
            print("    当前自私玩家的平衡开机时间：%.2f"%PlayerOntimes[0][-1])
            if PlayerOntimes[0][-1] >= 0.02:
                print("当前存在非 1 的相对算力，退出！")
                FLAG = True
                break
        # 检测是否存在某一次“自私玩家的相对算力”不为 1
        if FLAG:
            ebrr += 0.5
        else:
            print("相对算力资源利用率全为 1 时，EBRR=%.1f"%ebrr)
            break
