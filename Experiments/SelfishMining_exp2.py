...
if __name__ == '__main__':
    ...
    # 自私玩家的不同开机时间
    selfishOntilesList = np.linspace(0, 1, 101)[:-1]

    # 自私玩家的开机时间、开机比例、总矿机数
    selfish_OnProportions = [1]
    selfish_Total = 32

    # 诚实玩家的开机时间、开机比例、总矿机数
    honest_OnTimes = [[0], [0.4], [0.8], [0, 0.4, 0.8]]
    honest_OnProportions = [[1], [1], [1], [0.2587, 0.2587, 0.4286]]
    honest_Total = 224
    ...
    selfishCnt = 1
    for selfishOntime in selfishOntilesList:
        # 遍历四种策略
        for index in range(len(honest_OnTimes)):
            obj = SelfishMining([selfishOntime], selfish_OnProportions, selfish_Total, honest_OnTimes[index],
                                honest_OnProportions[index], honest_Total, gamma, STEPS, LIMIT)
            # 计算转移概率
            ...
            # 依次记录图 1、2、3 的数据
            for opIndex, C_op in enumerate(C_op_List):
                # 设置奖励参数
                obj.rewardParameter(rewardExpCount=10000, EBRR=2, op=C_op, cop=OPL - C_op)
                # 计算绝对效用
                selfishUtility, honestUtility = obj.getUtility()
                # 记录
                if opIndex == 0:
                    exp1[index].append(selfishUtility)
                elif opIndex == 1:
                    exp2[index].append(selfishUtility)
                else:
                    exp3[index].append(selfishUtility)

            print("进度：{:.2f}%".format(selfishCnt / 4))
            selfishCnt += 1

    # 将结果写入文件
    RESULT = [exp1, exp2, exp3]
    for index, result in enumerate(RESULT):
        with open("../ExperimentData/exp2_" + str(index + 1) + ".csv", "w") as f:
            # 依次写入策略 1、2、3、4 得到的绝对效用数据
            for strategy in result:
                utilityNum = len(strategy)
                for utilityIndex, utility in enumerate(strategy):
                    if utilityIndex != utilityNum - 1:
                        f.write(str(utility) + ",")
                    else:
                        f.write(str(utility) + "\n")
    print("文件写入成功！")
