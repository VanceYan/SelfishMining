...
def findBestOntime(selfishMachineTotal, honestMachineTotal, EBRR, C_op, C_cop):
    '''
    诚实玩家固定开机时间为 0 时，自私玩家的最优开启时间
    :param selfishMachineTotal: 自私玩家机器总数
    :param honestMachineTotal: 诚实玩家机器总数
    :return:
    '''
    # SelfishMining 相关参数
    ...
    for PlayerOntime in PlayerOntimes:
        obj = SelfishMining([PlayerOntime], [1], selfishMachineTotal,
                            [0], [1], honestMachineTotal, gamma, steps, limit)
        ...
        # 计算绝对效用
        selfishUtility, honestUtility = obj.getUtility()
        # 记录最优效用及对应的开机时间
        if selfishUtility > bestUtility:
                bestUtility = selfishUtility
                bestOntime = PlayerOntime
    return bestOntime, selfishUtility


def getResult(EBRR, C_op, C_cop, savePath, machineTotal = 256):
    '''
    自私玩家以及诚实玩家的最优开启时间
    :param C_op:
    :param C_cop:
    ...
    '''
    # 自私玩家的相对算力
    selfishPortions = np.linspace(0.01, 0.5, 50)
    ...
    for selfishPortion in selfishPortions:
        ...
        # 自私玩家的机器总数
        selfishMachineTotal = machineTotal * selfishPortion
        # 诚实玩家的机器总数
        honestMachineTotal = machineTotal - selfishMachineTotal
        # 寻找自私的最优开机时间
        bestOntime, selfishUtility = findBestOntime(selfishMachineTotal, honestMachineTotal, EBRR, C_op, C_cop)
        print(" 最优开机时间、效用分别为：%.2f、 %.6f\n"%(bestOntime, selfishUtility))
        bestOntimeList.append(bestOntime)
    # 记录实验数据
    with open(savePath, "a") as f:
        lineLen = len(bestOntimeList)
        for index, onTime in enumerate(bestOntimeList):
            if index != lineLen - 1:
                f.write(str(round(onTime, 2))+",")
            else:
                f.write(str(round(onTime, 2))+"\n")
...
if __name__ == '__main__':
    # 实验 8
    # 图 1
    for ebrr in [0, 0.5, 1, 1.5, 2]:
        getResult(ebrr, 0.02, 0, "../ExperimentData/exp8_1.csv")
    # # 图 2
    # for ebrr in [0, 0.5, 1, 1.5, 2]:
    #     getResult(ebrr, 0.01, 0.1, "../ExperimentData/exp8_2.csv")
    # # 图 3
    # for ebrr in [0, 0.5, 1, 1.5, 2]:
    #     getResult(ebrr, 0, 0.02, "../ExperimentData/exp8_3.csv")
