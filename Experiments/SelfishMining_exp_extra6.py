...
def findBestOntimeForSelfish(selfishPortion, machineTotal, EBRR, C_op, C_cop):
    '''
    默认诚实玩家开机时间为 0 ，寻找自私玩家的最优开机时间及对应的效用
...
def getResult(C_op, C_cop, savePath, machineTotal = 256):
    ...
    for selfishPortion in selfishPortions:
        ...
        for index, ebrr in enumerate(EBRR):
            # 寻找最优开机时间以及对应的效用
            bestOntime, bestUtility = findBestOntimeForSelfish(selfishPortion, machineTotal,
                                         ebrr, C_op, C_cop)
            print("EBRR=%d" % ebrr)
            print("最优开机时间：%.2f" % bestOntime)
            print("最优情况效用：%.6f" % bestUtility)
            # 保存结果
            ANS[index].append([bestOntime, bestUtility])
    # 将结果写入文件
    with open(savePath, "w") as f:
        for index, ebrr in enumerate(EBRR):
            ...
            for i in range(datasLen - 1):
                f.write(str(round(datas[i][0], 2)) + ",")
            f.write(str(round(datas[-1][0], 2)) + "\n")
            for i in range(datasLen - 1):
...
if __name__ == '__main__':
    ...
    OP = [0.02, 0.01, 0, 0.02, 0.01]
    COP = [0, 0.01, 0.02, 0.01, 0.02]
    for i in range(len(OP)):
        getResult(OP[i], COP[i], "../ExperimentData/exp_extra6_"+ str(i+1))
