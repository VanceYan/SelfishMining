...
def printUtility(Title, Utility, EBRRList):
    print()
    print(Title)
    for index, utility in enumerate(Utility):
        print("EBRR=%.1f "%EBRRList[index] + str(utility))
...
if __name__ == '__main__':
    ...
    # Cop 与 Ccop
    OPL = 0.02
    C_op_List = [0.02, 0.01, 0]

    # 自私玩家的开机时间、开机比例、总矿机数
    selfish_OnTimes = [0, 0.2, 0.5]
    selfish_OnProportions = [0.5, 0.3, 0.2]

    # 诚实玩家的开机时间、开机比例、总矿机数
    honest_OnTimes = [0, 0.4, 0.8]
    honest_OnProportions = [0.6, 0.2, 0.2]
    ...
    for relative in RelativeList:
        # 自私玩家的矿机总数
        selfish_Total = relative * machine_Total
        # 诚实玩家的矿机总数
        honest_Total = machine_Total - selfish_Total
        # 构建挖矿对象
        obj = SelfishMining(selfish_OnTimes, selfish_OnProportions, selfish_Total,
                            honest_OnTimes, honest_OnProportions, honest_Total, gamma, STEPS, LIMIT)
        # 计算转移概率
        ...
        for EBRR in EBRRList:
            ...
            for C_op in C_op_List:
                # 设置奖励参数
                obj.rewardParameter(rewardExpCount=10000, EBRR=EBRR, op=C_op, cop=OPL - C_op)
                # 计算绝对效用
                selfishUtility, honestUtility = obj.getUtility()
                # 记录
                if opIndex == 0:
                    exp1[ebrIndex].append(selfishUtility)
                elif opIndex == 1:
                    exp2[ebrIndex].append(selfishUtility)
                else:
                    exp3[ebrIndex].append(selfishUtility)
                opIndex += 1
            ebrIndex += 1
    ...
    printUtility("实验一图 1 数据", exp1, EBRRList)
    printUtility("实验一图 2 数据", exp2, EBRRList)
    printUtility("实验一图 3 数据", exp3, EBRRList)
