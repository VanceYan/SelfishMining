...
class SelfishMining:
    ...
    def __getMeanBaseWardAndExpense(self):
        ...
        print("\n%d 次随机取数得到的 [下次区块生成时间] 均值为：%.4f" % (self.rewardExpCount, averageT))
...
if __name__ == '__main__':
    ...
    # 设置相关参数（步长 Steps 在 200 左右取得的均方误差最小, limit 取值不应低于 1000）
    gamma, steps, limit, expCnt = 0.5, 200, 1000, 10000
    # 构建自私挖矿对线
    obj = SelfishMining([0, 0.6], [0.5, 0.5], 128,
                        [0.3, 0.9], [0.5, 0.5], 128,
                        weight=gamma, steps=steps, expCount=limit)
    # 计算转移概率
    obj.getTransferProbability(True)
    # 进行马尔可夫状态转移
    obj.getMarkovState()
    # 绘制概率密度函数
    obj.drawPDF()
    # 绘制概率分布函数
    obj.drawCDF()
    # 设置奖励参数（带缺省值，包括：EBRR、C_op、C_cap等）
    obj.rewardParameter(expCnt)
    # 计算效用
    obj.getUtility(True)
